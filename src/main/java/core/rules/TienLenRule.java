...
public class TienLenRule implements RuleSet {
	...
	public static CombinationType getCombinationType(List<Card> cards) {
		...
		case 6:
			if (isStraight(sortedCards))
				return CombinationType.STRAIGHT;
			if (isThreePairStraight(sortedCards))
		...
		case 8:
			if (isStraight(sortedCards))
				return CombinationType.STRAIGHT;
			if (isFourPairStraight(sortedCards))
	...
	public boolean canPlayAfter(List<Card> newCards, List<Card> previousCards) {
		...
		if (newType == CombinationType.FOUR_OF_KIND && prevType == CombinationType.SINGLE
				&& getTienLenValue(sortedPreviousCards.get(0)) == 15) {
			return true;
		}
		if (newType == CombinationType.THREE_PAIR_STRAIGHT) {
			if (prevType == CombinationType.SINGLE && getTienLenValue(sortedPreviousCards.get(0)) == 15)
				return true;
			if (prevType == CombinationType.PAIR && getTienLenValue(sortedPreviousCards.get(0)) == 15)
				return true;
			if (prevType == CombinationType.THREE_PAIR_STRAIGHT) {
				return getTienLenValue(sortedNewCards.get(sortedNewCards.size() - 1)) > getTienLenValue(
						sortedPreviousCards.get(sortedPreviousCards.size() - 1));
			}
		}
		if (newType == CombinationType.FOUR_PAIR_STRAIGHT) {
			...
			if (prevType == CombinationType.PAIR && getTienLenValue(sortedPreviousCards.get(0)) == 15)
				return true;
			if (prevType == CombinationType.FOUR_OF_KIND)
				return true;
			if (prevType == CombinationType.THREE_PAIR_STRAIGHT)
				return true;
	...
	private static int getSuitValue(Card.Suit suit) {
		return switch (suit) {
		case SPADES -> 0; // Bích (nhỏ nhất)
		case CLUBS -> 1; // Chuồn
		case DIAMONDS -> 2; // Rô
		case HEARTS -> 3; // Cơ (lớn nhất)
		};
	}
